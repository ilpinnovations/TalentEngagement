 ...
 */
public class UltimatixDetailsActivity extends AppCompatActivity {
    private final int id = 3;
    private TextView ultimatix;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.ultimatix_details_activity);

        if (getSupportActionBar() != null) {
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
            getSupportActionBar().setTitle("Whats Where On Ultimatix");

        }

        ultimatix = (TextView) findViewById(R.id.ultimatix);

        ultimatix.setText(
                Html.fromHtml(
                        "<a href=\"https://www.ultimatix.net\">Go to Ultimatix</a> "));
        ultimatix.setMovementMethod(LinkMovementMethod.getInstance());
